...
# ===== B 모델 (RandomForest - is_strike 예측 → 그대로 k로 사용) =====
...
for col in cat_cols:
    if col in X_B.columns:
        X_B[col] = X_B[col].astype("category").cat.codes
        test_B[col] = test_B[col].astype("category").cat.codes
...
val_pred_B = model_B.predict_proba(X_val_B)[:, 1]
print(f"[B 모델(RandomForest) LogLoss] val: {log_loss(y_val_B, val_pred_B):.5f}")
print(f"[B 모델(RandomForest) ROC AUC] val: {roc_auc_score(y_val_B, val_pred_B):.5f}")

test_pred_B = model_B.predict_proba(test_B)[:, 1]
...
save_path = "submissions/submission_rf_lightgbm.csv"
